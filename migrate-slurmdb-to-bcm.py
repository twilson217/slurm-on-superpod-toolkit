...
def update_slurm_conf(primary_headnode: str, secondary_headnode: str, skip_confirm: bool = False) -> bool:
    ...
    BCM's autogenerated section doesn't always set these correctly when moving
    slurmdbd to head nodes. This function directly edits slurm.conf to ensure
    the correct values are set outside the autogenerated section.
    ...
    print(f"\n{'=' * 65}")
    print("UPDATING SLURM.CONF ACCOUNTING SETTINGS")
    print('=' * 65)
    
    print(f"\nPlanned changes to {slurm_conf_path}:")
    ...
    if not skip_confirm:
        answer = input("\nApply these slurm.conf changes? [y/N]: ").strip().lower()
        if answer not in ('y', 'yes'):
    ...
    try:
        ...
        # Find the autogenerated section markers
        autogen_start = None
        autogen_end = None
        ...
        # and add new ones after AccountingStorageUser
        new_lines = []
        added_settings = False
        
        for i, line in enumerate(lines):
            # Skip AccountingStorageHost/BackupHost lines that are outside autogen section
            # (but keep commented ones)
            ...
            # Remove existing uncommented AccountingStorageHost/BackupHost settings
            if line_stripped.startswith('AccountingStorageHost='):
                print(f"  Removing existing: {line_stripped}")
                continue
            ...
            # Add our settings after AccountingStorageUser line
        ...
        if not added_settings:
            ...
            # Insert before autogenerated section
            if autogen_start:
