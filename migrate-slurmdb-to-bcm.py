...
def update_slurm_conf(primary_headnode: str, secondary_headnode: str, skip_confirm: bool = False) -> bool:
    """Update slurm.conf with correct AccountingStorageHost and AccountingStorageBackupHost.
    
    This function checks if BCM's autogenerated section already has the correct values.
    If so, it only cleans up any duplicate entries outside the autogenerated section.
    If not, it adds the correct values outside the autogenerated section as a fallback.
    ...
    try:
        ...
        if bcm_handles_it:
            print(f"\n  ✓ BCM autogenerated section already has correct values:")
            print(f"    AccountingStorageHost={primary_headnode}")
            if secondary_headnode:
                print(f"    AccountingStorageBackupHost={secondary_headnode}")
            print(f"\n  Cleaning up any duplicate entries outside autogenerated section...")
        else:
            print(f"\n  BCM autogenerated section does not have correct values.")
            print(f"  Will add settings outside autogenerated section as fallback.")
            print(f"    AccountingStorageHost      : {primary_headnode}")
            if secondary_headnode:
                print(f"    AccountingStorageBackupHost: {secondary_headnode}")
        
        if not skip_confirm:
            answer = input("\nProceed with slurm.conf update? [y/N]: ").strip().lower()
            if answer not in ('y', 'yes'):
                print("Skipping slurm.conf update.")
                return False
        
        # Process lines: remove existing AccountingStorageHost/BackupHost outside autogen section
        # Only add new ones if BCM's autogen section doesn't have them correct
        new_lines = []
        added_settings = False
        removed_any = False
        
        for i, line in enumerate(lines):
            # Inside autogenerated section - keep as is
            ...
            # Remove existing uncommented AccountingStorageHost/BackupHost settings outside autogen
            if line_stripped.startswith('AccountingStorageHost='):
                print(f"  Removing duplicate: {line_stripped}")
                removed_any = True
                continue
            if line_stripped.startswith('AccountingStorageBackupHost='):
                print(f"  Removing duplicate: {line_stripped}")
                removed_any = True
                continue
            ...
            # Only add our settings if BCM doesn't handle it correctly
            if not bcm_handles_it:
                if line_stripped.startswith('AccountingStorageUser=') and not added_settings:
                    new_lines.append(f"AccountingStorageHost={primary_headnode}\n")
                    if secondary_headnode:
                        new_lines.append(f"AccountingStorageBackupHost={secondary_headnode}\n")
                    added_settings = True
                    print(f"  Added: AccountingStorageHost={primary_headnode}")
                    if secondary_headnode:
                        print(f"  Added: AccountingStorageBackupHost={secondary_headnode}")
        
        # Fallback: if we needed to add but couldn't find AccountingStorageUser
        if not bcm_handles_it and not added_settings:
            print("  ⚠ Could not find AccountingStorageUser line to insert after")
            print("    Appending to end of file before autogenerated section")
            if autogen_start:
                new_lines.insert(autogen_start, f"AccountingStorageHost={primary_headnode}\n")
                if secondary_headnode:
                    new_lines.insert(autogen_start + 1, f"AccountingStorageBackupHost={secondary_headnode}\n")
                added_settings = True
        ...
        if bcm_handles_it:
            if removed_any:
                print(f"  ✓ Cleaned up duplicate entries in {slurm_conf_path}")
            else:
                print(f"  ✓ No changes needed - {slurm_conf_path} already correct")
        else:
            print(f"  ✓ Updated {slurm_conf_path}")
        return True
        
    except Exception as e:
        print(f"  ✗ Failed to update slurm.conf: {e}")
        return False
